...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        int leftNode = top * 2 + 1;
        int rightNode = top * 2 + 2;
        /*
         * We have to ensure the precondition that left <= node. Otherwise, in
         * sift down method, it will be recognized as process end.
         */
        //if size of tree is greater than one
        //compare root with smaller or 2 children
        if (rightNode <= last) {

            if (order.compare(array[leftNode], array[rightNode]) <= 0
                    && order.compare(array[top], array[leftNode]) > 0) {
                //swap root w smaller child
                exchangeEntries(array, top, leftNode);
                //recursively sift down root to corresponding subtree
                siftDown(array, leftNode, last, order);
            }

            if (order.compare(array[rightNode], array[leftNode]) <= 0
                    && order.compare(array[top], array[rightNode]) > 0) {
                //swap root w smaller child
                exchangeEntries(array, top, rightNode);
                //recursively sift down root to corresponding subtree
                siftDown(array, rightNode, last, order);
            }
        } else if (leftNode <= last && rightNode > last) {
            if (order.compare(array[top], array[leftNode]) > 0) {
                exchangeEntries(array, top, leftNode);
                siftDown(array, leftNode, last, order);
            }
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        if (left < array.length) {
            //recursively heapify subtree of array
    ...
    public final void add(T x) {
        ...
        //enqueue x into sorting machine
    ...
    public final void changeToExtractionMode() {
        ...
        //swap from insertion mode to extraction by making false
    ...
    public final T removeFirst() {
        ...
        T index = this.heap[0];
        /*
         * We have to use exchangeEntries method to exchange the smallest with
         * the first value
         */
        exchangeEntries(this.heap, 0, this.heapSize - 1);
        this.heapSize--;
        //sift down roots, and order properly
    ...
    public final int size() {
        ...
        // If it is insertion mode, the length should be equal to length of entries.
        if (this.insertionMode) {
            size = this.entries.length();
        }
        assert this.conventionHolds();
        //return size of heaps
        return size;
    }
